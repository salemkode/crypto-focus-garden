...
export const compile = (): void => {
	...
	if (fs.existsSync(cacheFile)) {
		compilationCache = JSON.parse(
			fs.readFileSync(cacheFile, { encoding: "utf-8" }),
		);
	}
	...
	const result = fs
		...
		.map((fn) => ({
			...
			contents: fs.readFileSync(
				new URL(urlJoin(contractsDirectory.toString(), fn)),
				{ encoding: "utf-8" },
			),
		}));
	...
	result.forEach(({ fn, contents }) => {
		...
		if (!compilationCache[fn] || compilationCache[fn].mtime !== mtime) {
			...
			exportArtifact(
				artifact,
				new URL(
					`../artifacts/${fn.replace(".cash", ".artifact.ts")}`,
					import.meta.url,
				),
			);
	...
	fs.writeFileSync(cacheFile, JSON.stringify(compilationCache, null, 2), {
		encoding: "utf-8",
	});
};
